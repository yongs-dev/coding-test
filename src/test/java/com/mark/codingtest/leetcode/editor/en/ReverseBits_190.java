...
//
//
// Related Topics Divide and Conquer Bit Manipulation 👍 5086 👎 1433
...
public class ReverseBits_190 {
    ...
    @Test
    void test() {
        log.info("result: {}", new Solution().reverseBits(0B00000010100101000001111010011100));
    }
    ...
    public class Solution {
        ...
        public int reverseBits(int n) {
            int result = 0;
            for (int i = 0; i < 32; i++) {
                result = result << 1; // result를 왼쪽으로 시프트
                result = result | (n & 1); // n의 가장 낮은 비트를 result에 추가
                n = n >>> 1; // n을 오른쪽으로 시프트
            }
            return result;
        }
